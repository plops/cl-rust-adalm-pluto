...
impl<'ui> Ui<'ui> {
        pub fn draw_image<'p> (&self, texture_id: &'p usize, size: &'p [f32, 2]) -> Image<'ui,'p>{
...
fn main (){
        ...
        let empty_texture  = glium::texture::Texture2d::empty_with_format(&system.display, glium::texture::UncompressedFloatFormat::U8U8U8U8, glium::texture::MipmapsOption::NoMipmap, 8192, 128).unwrap();
        let texture_id  = imgui::render::renderer::TextureId::from((empty_texture.get_id() as usize));
        system.main_loop(move | _,  ui|{
                ...
                ui.text(format!("mouse: ({:.1},{:.1})", mouse_pos[0], mouse_pos[1]));
                        ...
                        Window::new(im_str!("texture")).size([2.00e+2, 1.00e+2], Condition::FirstUseEver).build(ui, ||{
